...
\clearpage
\section{第 4.2 节教材例题、习题与订正}
\label{exercise-set:QM-C04-S02}

\exercisemeta
  {QM-C04-S02-EX}
  {2026-09-10}
  {Shankar，例 4.2.4 与练习 4.2.2--4.2.3；印刷页 pp.~134--139；PDF pp.~146--151}
  {高斯波包、实波函数的平均动量与位置相位造成的动量平移}
  {例题与两道习题的位置表象、动量表象计算均已核对}

\exercisequestion{QM-C04-S02-E00}{教材例 4.2.4：高斯波包的位置与动量分布}

\textbf{对应知识点：}\relatedtopic{QM-C04-S02-T06}{高斯波包与表象}。

\textbf{题意。}给定 $\psi(x)=N\exp[-(x-a)^2/(2\Delta^2)]$，其中 $a\in\mathbb R$、$\Delta>0$。求归一化常数、动量波函数、位置和动量的均值及不确定度。教材位置：印刷页 pp.~134--138，PDF pp.~146--150。

\begin{checkedsolution}
由 Gaussian 积分，归一化要求 $|N|^2\sqrt\pi\Delta=1$。取 $N$ 为正实数，得到
\begin{equation}
  \psi(x)
...
\[
  ...
  \langle y^2\rangle
  =\frac1{\sqrt\pi\Delta}\int_{-\infty}^{\infty}
    y^2\ee^{-y^2/\Delta^2}\dd y
  =\frac{\Delta^2}{2}.
\]
因此位置均值与不确定度为
\begin{equation}
...
\end{equation}
这里 $\Delta$ 是波函数的宽度参数，并不等于位置标准差 $\Delta X$。位置平移仅给动量波函数增加相位，不改变动量概率密度。
\end{checkedsolution}
